...
func validateConfig(cfg *Config) error {
	...
	validSort := map[string]bool{"state": true, "local_ip": true, "remote_ip": true, "port": true, "process": true, "duration": true}
	if !validSort[cfg.SortBy] {
		return &ConfigError{
			Field: "sort",
			Value: cfg.SortBy,
			Err:   errors.New("must be one of: state, local_ip, remote_ip, port, process, duration"),
		}
	}
...
func filterSockets(sockets []Socket, state, localIP, remoteIP, process string, port int, watchFin bool) ([]Socket, error) {
	if state == "" && localIP == "" && remoteIP == "" && port == 0 && process == "" && !watchFin {
		return sockets, nil
	}
...
func sortSockets(sockets []Socket, sortBy string, finTracker *FinTracker) []Socket {
	sort.Slice(sockets, func(i, j int) bool {
		...
		case "duration":
			durI, _ := finTracker.GetDuration(sockets[i])
			durJ, _ := finTracker.GetDuration(sockets[j])
			return durI > durJ
		default:
			return false
		}
...
func displayConnections(sockets []Socket, format string, noColor bool, watchMode bool, showStats bool, showDurations bool) {
	...
	for _, s := range sockets {
		if len(s.State) > widths[0] {
			widths[0] = len(s.State)
		}
		...
		if len(s.Process) > widths[3] {
			widths[3] = len(s.Process)
		}
...
func headersToInterface(headers []string) []interface{} {
...
func processCycle(cfg *Config, procManager *ProcessManager, finTracker *FinTracker, dnsResolver *DNSResolver) error {
	...
	filteredSockets, err := filterSockets(sockets, cfg.State, cfg.LocalIP, cfg.RemoteIP, cfg.Process, cfg.Port, cfg.WatchFin)
	if err != nil {
		return err
	}
...
func main() {
	...
	sortBy := flag.String("sort", "state", "Sort by: state, local_ip, remote_ip, port, process, duration")
